...
def process_all():
	for foldername in [FULL_FOLDER,NAT_FOLDER,STATE_FOLDER,METRO_FOLDER]:
		...
		for filename in excel_files:
			...
			for data_type_code in ["03", "04", "01","13"]:
				print(data_type_code)
				...
				df = sum_groups_df(df,data_type_code)
...
def insert_data(df, year, data_type_code):
	...
	# agg_df = agg_df.astype(object).where(pd.notnull(agg_df), None) #but you need to make available the none
	agg_df = agg_df.fillna(9999999)
