...
def process_one_file(year,folder,ALL_URLS):
    ...
    elif (folder==NAT_FOLDER or folder==STATE_FOLDER):
        if int(year)<=2000:
            return
        process_zip_NAT_STATE(zip_path)
    else:
...
def process_zip_NAT_STATE( zip_path ):
    xlsx_path = zip_path.replace('.zip','.xlsx')
    with zipfile.ZipFile(zip_path, 'r') as zipp:
        to_extract = max( zipp.infolist() ,key=lambda x: x.file_size )
        with zipp.open(to_extract.filename) as f:
